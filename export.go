...
func exportAsCSV(txChan chan *ethrpc.Transaction) {
	...
	for tx := range txChan {
		w.WriteString(
			fmt.Sprintf(
				"%s,%d,%s,%d,%d,%s,%s,%d,%d,%d,%s\n",
				tx.Hash, tx.Nonce, tx.BlockHash, tx.BlockNumber, tx.TransactionIndex, tx.From, tx.To, tx.Value.Int64(), tx.Gas, tx.GasPrice.Int64(), tx.Input,
			),
		)
		w.Flush()
	}
